...
- `sys._onProgress` callback: set by callers (e.g. term-view.js) before boot completes; wc.js calls it with `{type,path,loaded,total}` for `opfs-init` and `desktop-init` worker messages; callers must clear it (`sys._onProgress = null`) after boot to avoid stale refs; term-view.js writes `\r\x1b[2KLoading <path>: <n>/<total>` to xterm during boot wait, then clears the line
